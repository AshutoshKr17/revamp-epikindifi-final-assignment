...
const book_row =(book) =>{
    ...
    if(current_user == book.borrower && isUserLoggedIn()){
        action = `<button class="button" onclick=return_book(${book.id})>Return</button>`;
    }
    ...
    {
        action = `<button class="button" onclick="borrow_book(${book.id})">Borrow</button>`;
    }
    return '<tr>'+
...
const fetchInputRow=() =>{  
    ...
    if(isUserLoggedIn()){
        inputRow.innerHTML  =   '<tr>'+
                                    '<td id="new-book-id"><a href="#">'+book_count+'</a></td>'+
                                    '<td>'+'<input class="form-control-sm"  id="new-book-title" placeholder="Title">'+'</td>'+
                                    '<td>'+'<input class="form-control-sm"  id="new-book-author" placeholder="Author">'+'</td>'+
                                    '<td id="lender">'+current_user+'</td>'+
                                    '<td id="borrower">-</td>'+
                                    '<td id="button-action-new-book">'+'<button class="button" onclick="insert_new_book()">Add Book</button>'+'</td>'+
                                '</tr>';
    }
}
